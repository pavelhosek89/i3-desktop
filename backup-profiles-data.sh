...
if [ -d "$BACKUP_FOLDER" ]; then
	if [ -f "$FILE_ETC" ]; then
		echo -e "\n\e[1;31mETC\e[0m"
		sudo rsync -ahrv --delete --dry-run --files-from=$FILE_ETC \
		    /etc/ $BACKUP_FOLDER/etc/
	fi

	if [ -f "$FILE_OPT" ]; then
		echo -e "\n\e[1;31mOPT\e[0m"
		sudo rsync -ahrv --delete --dry-run --files-from=$FILE_OPT \
		    /opt/ $BACKUP_FOLDER/opt/
	fi

	if [ -f "$FILE_PROFILES" ]; then
		echo -e "\n\e[1;31mPROFILES\e[0m"
		sudo rsync -ahrv --delete --dry-run --files-from=$FILE_PROFILES \
